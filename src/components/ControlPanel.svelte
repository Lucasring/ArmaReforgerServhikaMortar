<script>
  ...
  import { MAP_SCALE_METERS_PER_PIXEL, mortarTypes } from '../config/mortarConfig';
  ...
  function findAppropriateRing(range, ballistics) {
    ...
    for (let i = 0; i < ballistics.rings.length; i++) {
      ...
      console.log('Checking ring', i, 'range:', minRange, 'to', maxRange, 'for target range:', range);
      ...
      if (range >= minRange && range <= maxRange) {
        ...
        if (range >= minRange + rangeBuffer && range <= maxRange - rangeBuffer) {
          rangeWarning = '';
    ...
    if (highestRing >= 0 && ballistics.rings[highestRing] && ballistics.rings[highestRing].length >= 2) {
  ...
  function handleMortarTypeChange(event) {
    const newMortarType = mortarTypes.find(m => m.name === event.target.value);
    if (newMortarType) {
  ...
  function handleAmmoTypeChange(event) {
    if (selectedMortarType && selectedMortarType.ammo) {
      const newAmmoType = selectedMortarType.ammo.find(a => a.name === event.target.value);
      if (newAmmoType) {
  ...
  function handleAutoSelectChange(event) {
    ...
    if (autoSelectRing && distance !== null && selectedAmmoType && selectedAmmoType.ballistics) {
      const appropriateRing = findAppropriateRing(distance, selectedAmmoType.ballistics);
  ...
  function interpolateElevation(range, ballistics) {
    ...
    console.log('Interpolating elevation for range:', range, 'using ring data:', ringData);
    
    const sortedData = [...ringData].sort((a, b) => a.range - b.range);
    let nearestPoints = sortedData.map(point => ({
      ...point,
    ...
    const elevationMils = lower.elevationMils + ratio * (upper.elevationMils - lower.elevationMils);
    ...
    if (mortarPosition && targetPosition) {
      ...
      if (newDistance !== distance) {
        ...
        if (autoSelectRing && selectedAmmoType && selectedAmmoType.ballistics) {
          ...
          if (appropriateRing !== null) {
            selectedRing = appropriateRing;
            console.log('Auto-selected ring:', appropriateRing, 'for distance:', distance);
            dispatch('ringChange', appropriateRing);
          }
        }
      }
...
<div class="control-panel">
  ...
  <div class="selectors">
    ...
    <div class="selector">
      ...
      <select id="ring-select" bind:value={selectedRing} on:change={handleRingChange}>
  ...
  <div class="calculations">
    ...
    {#if elevation}
      <div class="calculation">
        ...
        <span class="value">{elevation.elevationMils} mils ({elevation.elevationDeg}°)</span>
      </div>
      <div class="calculation">
  ...
  <div class="instructions">
    ...
    <ul>
      ...
      <li>Enable auto-select ring to automatically choose the best ring for the current range</li>
